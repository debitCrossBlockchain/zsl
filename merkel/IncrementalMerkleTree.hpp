...
#include<map>
using namespace std;
std::vector<unsigned char> convertIntToVectorLE_(const uint64_t val_int);
std::vector<bool> convertBytesVectorToVector_(const std::vector<unsigned char>& bytes);
uint64_t convertVectorToInt_(const std::vector<bool>& v);
namespace libzcash {
	...
	inline void SerializationOp(Stream& s, Operation ser_action) {
		...
		if (ser_action.ForRead()) {
			...
			for (size_t i = 0; i < pathBytes.size(); i++) {
				us.authentication_path.push_back(convertBytesVectorToVector_(pathBytes[i]));
			...
			indexInt = convertVectorToInt_(index);
			READWRITE(pathBytes);
			READWRITE(indexInt);
		}
	}
